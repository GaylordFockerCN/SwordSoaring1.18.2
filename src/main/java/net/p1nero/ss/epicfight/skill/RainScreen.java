    ...
    public void onInitiate(SkillContainer container) {
        ...
        listener.addEventListener(PlayerEventListener.EventType.SERVER_ITEM_USE_EVENT, EVENT_UUID, (event) -> {
            ...
            if(caster.hasStamina(10)){
                ...
                double r = 2;//伤害范围
                List<Entity> entities = player.getLevel().getEntities(player,new AABB(player.getPosition(0).add(-r,-r,-r), player.getPosition(0).add(r,r,r)));
                for (Entity entity : entities){
                    ...
                    if (toSourceLocation.dot(viewVector) > 0.0) {
                            ...
                            player.level.playSound(null, entity.getOnPos(), EpicFightSounds.BLADE_HIT, SoundSource.BLOCKS,1f,1f);
                    }
        ...
        listener.addEventListener(PlayerEventListener.EventType.HURT_EVENT_PRE, EVENT_UUID, (event) -> {
            ...
            if(ssPlayer.getSwordScreenEntityCount() > 0 && !(ssPlayer.isProtectNextFall() && damageSource.isFall())){

                Set<Integer> swordID = ssPlayer.getSwordScreensID();
                //剑挡伤害并回血（顺序很重要，需要先销毁剑再反弹，省得获取SourceEntity为null的时候被打断）
                for(int i : swordID){
                    Entity sword = serverPlayer.getLevel().getEntity(i);
                    if(sword != null){
...
//                        serverPlayer.serverLevel().playSound(null, serverPlayer.getX(), serverPlayer.getY(), serverPlayer.getZ(), SoundEvents.EXPERIENCE_ORB_PICKUP, SoundSource.BLOCKS, 1, 1);
                ...
                EpicFightParticles.HIT_BLUNT.get().spawnParticleWithArgument(serverPlayer.getLevel(), HitParticleType.FRONT_OF_EYES, HitParticleType.ZERO, serverPlayer, damageSource.getDirectEntity());
    ...
    public static void summonSword(ServerPlayer player){
        ...
        player.getCapability(SSCapabilityProvider.SS_PLAYER).ifPresent(ssPlayer -> {
            ...
            for(int i = 0; i < 4; i++){
                    RainScreenSwordEntity sword = (RainScreenSwordEntity) ModEntities.RAIN_SCREEN_SWORD.get().spawn(player.getLevel(), null, null, player.getOnPos(), MobSpawnType.MOB_SUMMONED, true,true);
                    if(sword == null){
                        return;
                    }
...
//        GuiComponent.blit(poseStack, (int)x, (int)y, 24, 24, 0, 0, 1, 1, 1, 1);
        GuiComponent.drawString(poseStack ,gui.font, String.format("RS %d", (ssPlayer.rainScreenCooldownTimer / 40)), (int) (x + 4), (int) (y + 6), 16777215);
        poseStack.popPose();
    }
